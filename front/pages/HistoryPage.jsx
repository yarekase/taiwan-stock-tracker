...
// ====================================================================
  ...
  const profit = displayRecords.reduce((acc, cur) =>{
    ...
    const dataTotal = cur.price * cur.quantity;
    if (cur.type === "buy") {
      // 用records陣列裡的type來確認資料是買還是賣
      acc.totalBuy += dataTotal;
      acc.buyQuantity += cur.quantity;
      // 把cur的資料累加進去買入的acc裡
    }
    else {
      acc.totalSell += dataTotal;
        ...
        <tbody>  
          {displayRecords.length > 0 ? (
            displayRecords.map((item) => {
                ...
                // 用map需要把每個資料做一個key
              const dataTotal = item.price * item.quantity;
              ...
              <tr key={item.id}> 
                ...
                <td>{item.quantity}</td>
                <td>${dataTotal.toLocaleString()}</td>
                <td>
                  <button 
                    className="btn-delete" 
                    onClick={() =>{
                      console.log("當前要刪除的整筆資料物件:", item);
                      console.log("要傳給 App.jsx 的 id 值是:", item.id);
                      onDelete(item.id)}}>
                    刪除
                  </button>
                </td>
            ...
            <tr><td colSpan="7">目前尚無成交紀錄</td></tr>
            // colSpan是合併儲存格，合併七個格子，寫上目前尚無成交紀錄
          )}
        </tbody>
      </table>
