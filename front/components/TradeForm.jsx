...
import { postRecord } from '../src/api';
...
function TradeForm({ selectedStock, onSubmit }) {
  ...
  useEffect(() => {
    if (selectedStock) {
      ...
      setPrice(selectedStock.current_price || "0");
    //   當selectedStock發生變動時，用後端抓到的成交價格填入，後續可以在自己輸入
    }
  }, [selectedStock]);

  
  // 處理按下「買入」或「賣出」的邏輯，由於API請求是非同步的，因此要使用async
