...
// ==========================================================================
  ...
  // 當使用者輸入時，進行搜尋
  useEffect(() => {
    // 檢查stockList有東西，以及StockList是陣列，否則直接跳出
    ...
    const filtered = stockList.filter(stock =>{
      ...
      const sName = String(stock.stock_name);

      return sId.startsWith(query) || sName.includes(query)}
      ).slice(0,10);
    setResults(filtered);
        ...
        <ul className="results-dropdown">
          {results.map((stock) => (
            ...
            <li key={stock.stock_id} onClick={() => {
              ...
              <span className="stock-price">${stock.current_price ? Number(stock.current_price).toFixed(2):'--'}</span>
              {/* span不會換行，是行內標籤 */}
            </li>
          ))}
        </ul>
