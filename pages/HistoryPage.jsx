...
function HistoryPage({ records, onDelete }) { // 從 App 接收資料 (Prop)

  // --- 1. 定義 const (State) ---
  const [filterName, setFilterName] = useState(''); // 用來存使用者的篩選文字

  // --- 2.定義計算總損益的方法 ---
  const profit = records.reduce((acc, cur) =>{
  ...
  // --- 3.淨投資 ---
  const netInvestment = profit.totalBuy - profit.totalSell;


  // --- 4. 處理排序 ---
  // 過濾資料：根據搜尋文字過濾 records，並按日期從新到舊排序
  const displayRecords = records
    .filter(r => r.stockName.includes(filterName) || r.stockId.startsWith(filterName))
    .sort((a, b) => b.id - a.id); // 日期降序
